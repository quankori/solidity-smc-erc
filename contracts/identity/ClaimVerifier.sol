...
contract ClaimVerifier {
    ...
    function checkClaim(address _beneficiary, uint256 claimType)
        public
        view
        returns (bool claimValid)
    {
        ClaimHolder beneficiaryIdentity = ClaimHolder(_beneficiary);
        if (claimIsValid(beneficiaryIdentity, claimType)) {
            // emit ClaimValid(beneficiaryIdentity, claimType); // Remove view type function
            return true;
        } else {
            // emit ClaimInvalid(beneficiaryIdentity, claimType); // Remove view type function
            return false;
        }
    }
